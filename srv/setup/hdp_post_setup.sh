...
if [ "$checkstate" = "Y" -o  "$checkstate" = "y" ]; then
  ...
  salt bigdata-hbase2 cmd.run "bash /root/rebuild_topic.sh"
  ...
  salt bigdata-hbase2 cmd.run "apt-get install -y at"
  salt bigdata-hbase2 cmd.run cwd=/usr/local/confluent/bin env='{"JAVA_HOME": "/usr/jdk64/jdk1.8.0_77"}' "./schema-registry-stop;"
  salt bigdata-hbase2 cmd.run cwd=/usr/local/confluent/bin env='{"JAVA_HOME": "/usr/jdk64/jdk1.8.0_77"}' "./schema-registry-start -daemon ../etc/schema-registry/schema-registry.properties|at now + 1 minutes exit 1 >/dev/null 2>&1" >/dev/null 2>&1
